...
#ifdef zmumujetsAna_LepSk_cxx

zmumujetsAna_LepSk::zmumujetsAna_LepSk(TTree *tree) : edimarcoTree(tree) {
...
void zmumujetsAna_LepSk::loop()
{
   ...
   selection twomuonsC("twomuonsC","2 muons");
   ...
   selection mumuInvMassC("mumuInvMassC","mumu mass in [60,120]");
   ...
   UInt_t maskMonoJetSelection = njetsEmanC.get2ToId() + jet1ptC.get2ToId() + jjdphiEmanC.get2ToId() +
                                   eLooseVetoC.get2ToId() + gammaLooseVetoC.get2ToId() + mumet200C.get2ToId();
   ...
   // mu_Acc_Eff.append(mu1etaC.get2ToId() + mu2etaC.get2ToId());
   // mu_Acc_Eff.append(mu1ptC.get2ToId() + mu2ptC.get2ToId() + mumuInvMassC.get2ToId());
   // mu_Acc_Eff.append(twomuLooseC.get2ToId());
   // mu_Acc_Eff.append(mu1tightIdC.get2ToId());

   mask mu_Acc_Eff("mumu acceptance and efficiency"); 
   mu_Acc_Eff.append(maskMuGen);
   mu_Acc_Eff.append(maskMonoJetSelection);
   mu_Acc_Eff.append(maskMuonAcceptance);
   mu_Acc_Eff.append(maskMuonEfficiency);
   ...
   zmumujetsControlSample.append(mumet200C.get2ToId());
   ...
   mask zmumujetsControlSampleGenMu("Z->mumu control sample (e generated) with selection flow as Emanuele's");
   ...
   zmumujetsControlSampleGenMu.append(mumet200C.get2ToId());
   ...
   tautaubkgInZmumu.append(mumet200C.get2ToId());
   ...
   //   mu_acc_eff[i]->append(mu1etaC.get2ToId() + mu2etaC.get2ToId());
   //   mu_acc_eff[i]->append(mu1ptC.get2ToId() + mu2ptC.get2ToId() + mumuInvMassC.get2ToId());
   //   mu_acc_eff[i]->append(twomuLooseC.get2ToId());
   //   mu_acc_eff[i]->append(mu1tightIdC.get2ToId());
   // }
   ...
   TH1D *HzmumujetsYieldsMetBin = new TH1D("HzmumujetsYieldsMetBin","yields of Zmumu control sample in bins of met;#slash{E}_{T};# of events",nMetBins,metBinEdges);
   ...
   TVector2 mu1, mu2, met, mu, muVectorSum;  // mu is any muon  (mu1 is first valid muon, mu2 is the second valid in the list sorted by pt)
   ...
   cout<<"zmumujetsAna_LepSk::loop()"<<endl;
   ...
   for (Int_t jentry=0; jentry<nentries; jentry++) {
     ...
     muVectorSum.SetMagPhi(0.0,0.0);   // for each event it must be initialized to 0

     // If I want, I can sum all the leptons with given PDG_ID to have metNoLep
     for (Int_t i = 0; i < nLepGood; i++) {
       if (fabs(LepGood_pdgId[i]) == MU_PDGID) {
	 mu.SetMagPhi(LepGood_pt[i],LepGood_phi[i]);
	 muVectorSum += mu;
       }
     }

     Double_t metNoMu_pt = (muVectorSum + met).Mod();
     ...
     eventMask += oppChargeMuonsC.addToMask( (LepGood_pdgId[firstIndex] * LepGood_pdgId[secondIndex]) == -169);  // |pdgID| = 13 for muons
     ...
     eventMask += mu1tightC.addToMask((LepGood_tightId[firstIndex] == 1) && (fabs(LepGood_pdgId[firstIndex]) == MU_PDGID) && (LepGood_relIso04[firstIndex] < 0.12 ) && (fabs(LepGood_eta[firstIndex]) < 2.4) && (LepGood_pt[firstIndex] > 20));
     eventMask += mu1tightIdC.addToMask((LepGood_tightId[firstIndex] == 1) && (LepGood_relIso04[firstIndex] < 0.12 ) && (fabs(LepGood_pdgId[firstIndex]) == MU_PDGID));
     ...
     eventMask += genMuonsC.addToMask( myPartGenAlgo(nGenPart, GenPart_pdgId, GenPart_motherId, MU_PDGID, 23) );
   ...
   Hacceff->SaveAs("hist_mumu_AccEff_metBin_LepSk.root");

   char answer = '\0';
   string fName = "zmumujetsAnaYields_LepSk.txt";
   ...
   if (answer == 'y') {
     ...
     if ( !myfile.is_open() ) {
       ...
       if ( system(command.c_str()) != 0) cout<<"Error during \"system(\""<<filename<<"\")\" call"<<endl;  
       myfile<<endl;
       myfile<<endl;
   ...
   const char *texfname = "zmumujetsAnaYields_LepSk.tex";
   ...
   const char *histFileName = "histZmumujetsAnaInvMass_LepSk.root";
   ...
   const char *histFileName2 = "histZmumujetsInvMassCS_ZJetsTomumu_LepSk.root";
   ...
   const char *histFileName3 = "histZmumujetsInvMassCS_ZJetsToTauTau_LepSk.root";
   ...
   const char *YieldsFileName = "histZmumujetsAnaYieldsMetBin_LepSk.root";
   ...
   HzmumujetsYieldsMetBinGenMu->SaveAs("histZmumujetsAnaYieldsMetBinGenMu_LepSk.root");
   ...
   HzmumujetsYieldsMetBinGenTau->SaveAs("histZmumujetsAnaYieldsMetBinGenTau_LepSk.root");
   ...
   HZtoMuMuRecoPt->SaveAs("histZmumuRecoPt_LepSk.root");
   HZtoMuMuGenPt->SaveAs("histZmumuGenPt_LepSk.root");
   HZtoMuMuPt_RecoGenRatio->Divide(HZtoMuMuRecoPt,HZtoMuMuGenPt);
   HZtoMuMuPt_RecoGenRatio->SaveAs("histZmumuPt_RecoGenRatio_LepSk.root");
   HZtoMuMuPt_RecoGenRatio_pdf->SaveAs("histZmumuPt_RecoGenRatio_pdf_LepSk.root");

   // save binned histograms in another file
   const char *histFileName4 = "histZmumuPt_RecoGenRatio_MetBin_LepSk.root";
   ...
   const char *histFileName5 = "histZmumuPt_RecoGenRatio_pdf_MetBin_LepSk.root";
   ...
   const char *histFileName6 = "histZmumuRecoPt_MetBin_LepSk.root";
   ...
   const char *histFileName7 = "histZmumuGenPt_MetBin_LepSk.root";
